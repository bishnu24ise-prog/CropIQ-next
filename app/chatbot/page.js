...
import { getAIReply, SUGGESTIONS } from "./chatbotData";
import { shareWhatsApp } from "../components/WhatsAppShare";

export default function ChatbotPage() {
  const [usingGemini, setUsingGemini] = useState(false);
  const [msgs, setMsgs] = useState([
    { type: "bot", html: '🙏 <strong>Namaste!</strong> I am <strong>KisaanBot</strong>, your AI farming advisor powered by CropIQ.<br><br>I can help you with:<br>🌾 Crop advice & sowing calendar<br>🐛 Pest & disease management<br>💧 Irrigation & fertilizer guidance<br>🏛️ Government schemes & subsidies<br>💰 Loan & financial planning<br><br>What would you like to know today?' }
  ]);
  ...
  async function send(text) {
    if (!text.trim()) return;
    setInput("");
    const userMsg = { type: "user", html: text, text, time: now() };
    setMsgs(p => [...p, userMsg]);
    setTyping(true);
    ...
    try {
      ...
      if (data.reply) {
        ...
        setTyping(false);
        setMsgs(p => [...p, { type: "bot", html: data.reply, text: data.reply, time: now() }]);
        return;
      }
    } catch {}
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .page-nav{background:rgba(6,78,59,0.9);backdrop-filter:blur(12px);padding:14px 30px;display:flex;align-items:center;justify-content:space-between}
        ...
        .chat-page{max-width:900px;margin:0 auto;padding:30px 20px}
        ...
        .ai-badge{display:inline-flex;align-items:center;gap:8px;background:rgba(46,213,115,0.2);border:1px solid rgba(46,213,115,0.5);color:#2ed573;padding:6px 16px;border-radius:30px;font-size:.75rem;font-weight:800;margin-bottom:12px}
        ...
        .chat-window{background:white;border-radius:24px;box-shadow:0 10px 40px rgba(0,0,0,0.08);border:1px solid #e5e7eb;overflow:hidden;display:flex;flex-direction:column;height:65vh}
        .chat-messages{flex:1;padding:20px;overflow-y:auto;display:flex;flex-direction:column;gap:14px;scroll-behavior:smooth}
        .msg{max-width:78%;padding:12px 16px;border-radius:18px;font-size:.9rem;line-height:1.6;animation:msgIn .3s ease}
        @keyframes msgIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
        .msg.bot{background:#f0fdf4;color:#1f2937;align-self:flex-start;border-bottom-left-radius:4px;border:1px solid #d1fae5}
        .msg.user{background:linear-gradient(135deg,#064e3b,#047857);color:white;align-self:flex-end;border-bottom-right-radius:4px}
        .msg-time{font-size:.6rem;opacity:.5;margin-top:4px}
        .bot-icon{width:28px;height:28px;background:#064e3b;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.85rem;margin-bottom:4px}
        ...
        .chip{background:#f0fdf4;color:#064e3b;border:1px solid #d1fae5;padding:7px 14px;border-radius:20px;font-size:.75rem;font-weight:700;cursor:pointer;transition:.2s;white-space:nowrap}
        .chip:hover{background:#064e3b;color:white}
        ...
        .chat-input:focus{border-color:#064e3b}
      ...
      <div className="chat-page">
        <div className="chat-header">
          <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginBottom:12}}>
            <div className="ai-badge"><div className="ai-dot" /> AI FARMING ADVISOR · ONLINE</div>
            {usingGemini && <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(99,102,241,0.2)",border:"1px solid rgba(99,102,241,0.5)",color:"#a5b4fc",padding:"6px 16px",borderRadius:30,fontSize:".75rem",fontWeight:800}}>✨ Gemini AI Active</div>}
          </div>
          <h1>🤖 KisaanBot — Your Smart Farming Guide</h1>
          <p>Ask any question about crops, weather, loans, pests, or government schemes.</p>
        </div>
        ...
        <div className="chat-window">
          <div className="chat-messages" ref={boxRef}>
            {msgs.map((m, i) => (
              <div key={i}>
                {m.type === "bot" && <div className="bot-icon">🤖</div>}
                <div className={`msg ${m.type}`} dangerouslySetInnerHTML={{ __html: m.html }} />
                <div style={{display:"flex",alignItems:"center",justifyContent:m.type==="user"?"flex-end":"flex-start",gap:8,marginTop:2}}>
                  {m.time && <div className="msg-time">{m.time}</div>}
                  {m.type === "bot" && i > 0 && <button onClick={()=>shareWhatsApp(`CropIQ KisaanBot says:\n${m.text||m.html?.replace(/<[^>]*>/g,"")}`)} style={{background:"none",border:"none",fontSize:".65rem",color:"#25D366",cursor:"pointer",fontWeight:700}}>💬 Share</button>}
                </div>
              </div>
            ))}
            {typing && (
